...
RCT_EXPORT_METHOD(sendMessage : (NSDictionary *)message : (NSString *)eventId) {
    RCMessage *resendMsg = [RCIMClient.sharedRCIMClient sendMessage:[message[@"conversationType"] intValue]
    ...
    if (resendMsg != nil) {
        [self sendEventWithName:@"rcimlib-send-message"
            ...
            @"type" : @"attached",
            @"eventId" : eventId,
            @"message" : [self fromMessage:resendMsg]
        }];
    } else {
...
RCT_EXPORT_METHOD(resendMessageById:(NSInteger)messageId
    ...
    if ([message.content isKindOfClass:[RCMediaMessageContent class]]) {
        ...
        resendMsg = [[RCIMClient sharedRCIMClient] sendMessage:message pushContent:pushContent pushData:pushData successBlock:^(RCMessage *successMessage) {
            [self sendEventWithName:@"rcimlib-send-message"
                               body:@{
                @"type" : @"success",
                @"eventId" : eventId,
...
RCT_EXPORT_METHOD(sendMediaMessage : (NSDictionary *)message : (NSString *)eventId) {
    RCMessage *resendMsg = [RCIMClient.sharedRCIMClient sendMediaMessage:[message[@"conversationType"] intValue]
      targetId:message[@"targetId"]
      content:[self toMessageContent:message[@"content"]]
      pushContent:message[@"pushContent"]
      pushData:message[@"pushData"]
...
RCT_EXPORT_METHOD(deleteRemoteMessages
  ...
  NSMutableArray *array = [NSMutableArray arrayWithCapacity:messages.count];
  for (int i = 0; i < messages.count; i += 1) {
    array[i] = [self toMessage:messages[i]];
  }
  [RCIMClient.sharedRCIMClient deleteRemoteMessage:conversationType targetId:targetId messages:array success:^{
    resolve(@(YES));
  } error:^(RCErrorCode status) {
    [self reject:reject error:status];
  }];
...
#pragma mark - PushContent

RCT_EXPORT_METHOD(setPushContentShowStatus:(BOOL)status
                  : (RCTPromiseResolveBlock)resolve
                  : (RCTPromiseRejectBlock)reject) {

  [[RCIMClient sharedRCIMClient].pushProfile updateShowPushContentStatus:status success:^{
    resolve(nil);
  } error:^(RCErrorCode status) {
    reject([@(status) stringValue], @"", nil);
  }];
...
RCT_EXPORT_METHOD(getPushContentShowStatus
                  : (RCTPromiseResolveBlock)resolve
                  : (RCTPromiseRejectBlock)reject) {
    resolve(@([RCIMClient sharedRCIMClient].pushProfile.isShowPushContent));
}
...
- (void)didReceiveReadReceiptNotification:(NSNotification *)notification {
  ...
  [self sendEventWithName:@"rcimlib-read-receipt-received" body:body];
}
...
- (void)sendAPNSArrivedEvent:(NSDictionary *)data {
  if ([data isKindOfClass:[NSNotification class]]) {
    NSNotification *notification = (NSNotification *)data;
    if ([notification.name isEqualToString:RC_APNS_NOTIFICATION_ARRIVED_EVENT]) {
      [self sendEventWithName:@"rcimlib-push-arrived" body:notification.object];
    }
  }
}

- (void)sendAPNSOpenEvent:(NSDictionary *)data {
  if ([data isKindOfClass:[NSNotification class]]) {
    NSNotification *notification = (NSNotification *)data;
    if ([notification.name isEqualToString:RC_APNS_NOTIFICATION_OPEN_EVENT]) {
      [self sendEventWithName:@"rcimlib-push-clicked" body:notification.object];
    }
  }
}
...
- (RCMessage *)toMessage:(NSDictionary *)messageDict {

  RCConversationType conversationType = [messageDict[@"conversationType"] integerValue];
  NSString *targetId = messageDict[@"targetId"];
  RCMessageDirection direction = [messageDict[@"messageDirection"] integerValue];
  RCMessageContent *content = [self toMessageContent:messageDict];

  RCMessage *message = [[RCMessage alloc] initWithType:conversationType targetId:targetId direction:direction content:content];
  message.objectName = messageDict[@"objectName"];
  message.messageUId = messageDict[@"messageUId"];
  message.messageId = [messageDict[@"messageId"] longValue];
  message.senderUserId = messageDict[@"senderUserId"];
  message.sentTime = [messageDict[@"sentTime"] longLongValue];
  message.sentStatus = [messageDict[@"sentStatus"] integerValue];
...
- (NSArray<NSString *> *)supportedEvents {
  return @[
    ...
    @"rcimlib-push-arrived",
    @"rcimlib-push-clicked"
  ];
}
